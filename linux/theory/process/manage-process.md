...
columns=`tput cols`
nc=`echo $(( $columns/2 ))`
rows=`echo $(( $(tput lines)/2 ))`

mid=`echo $(( $columns*$rows+$nc-4 ))`
...
Now set the execution permisiion by executing command _**` chmod +x clock.sh `**_ and execute the script as follow:
```
$ ./clock.sh
```
